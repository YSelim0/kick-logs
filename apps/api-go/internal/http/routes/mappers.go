...
func operationsSummaryResponse(summary domain.OperationsSummary) schemas.OperationsSummaryResponse {
	...
	return schemas.OperationsSummaryResponse{
		...
		Processor: schemas.ListenerHeartbeatResponse{
			ServiceName:          summary.Processor.ServiceName,
			LastSeenAt:           nullableTime(summary.Processor.LastSeenAt),
			IsFresh:              summary.Processor.IsFresh,
			StaleAfterSeconds:    summary.Processor.StaleAfterSeconds,
			SecondsSinceLastSeen: nullableListenerSeconds(summary.Processor),
		},
		Ingestion: schemas.IngestionHealthResponse{
			...
			LegacyQueueDepth:               summary.Ingestion.LegacyQueueDepth,
			LegacyOldestPendingAgeSeconds:  summary.Ingestion.LegacyOldestPendingAgeSeconds,
			CapturedRawEvents:              summary.Ingestion.CapturedRawEvents,
			RecentMessagePollCaptured:      summary.Ingestion.RecentMessagePollCaptured,
			RecentMessagePollErrors:        summary.Ingestion.RecentMessagePollErrors,
			StreamMessages:                 summary.Ingestion.StreamMessages,
			StreamBytes:                    summary.Ingestion.StreamBytes,
			StreamConsumerPending:          summary.Ingestion.StreamConsumerPending,
			StreamConsumerAckPending:       summary.Ingestion.StreamConsumerAckPending,
			StreamConsumerRedelivered:      summary.Ingestion.StreamConsumerRedelivered,
			StreamOldestPendingAgeSeconds:  summary.Ingestion.StreamOldestPendingAgeSeconds,
			StreamLatestMessageAgeSeconds:  summary.Ingestion.StreamLatestMessageAgeSeconds,
			StreamLatestConsumerUpdateTime: nullableTime(summary.Ingestion.StreamLatestConsumerUpdateTime),
			StreamError:                    summary.Ingestion.StreamError,
