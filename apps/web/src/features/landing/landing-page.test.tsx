...
describe("LandingPage", () => {
  ...
  it(
    "renders the v2 hero, stats bar, and analytics panels",
    async () => {
      analyticsMocks.getAnalyticsOverview.mockResolvedValue({
        total_messages: 482,
        total_senders: 76,
  ...
  it("keeps successful analytics visible when one panel fails", async () => {
    ...
    analyticsMocks.getMessageVolume.mockResolvedValue({
      items: [{ bucket_start: "2026-05-14T00:00:00Z", message_count: 60 }]
    });
    analyticsMocks.getTopChannels.mockRejectedValue(new Error("memory limit"));
    analyticsMocks.getTopEmotes.mockResolvedValue({
      items: [
        {
          id: "37226",
